...
def create_pattern(agv_data, shape = [2100, 2970], chessboard_corners = [CALIBRATION_CORNERS_X, CALIBRATION_CORNERS_Y]):
    ...
    chessboard = create_chess_board(chessboard_corners[0], chessboard_corners[1], 200)

    place_pattern_on_img(qr_code, img, [0,0])
    place_pattern_on_img(chessboard, img, [0, int(shape[1]/2)])

    cv2.imwrite(f'{DATA_DIR}/calibration_pattern.{CALIBRATION_IMGS_FORMAT}', img)
...
def place_pattern_on_img(pattern, img, pos):
    ...
    img[offset_x:offset_x+pattern_length, offset_y:offset_y+pattern_width] = pattern
...
def create_code(data, size = 1000):
    ...
    qr_array = cv2.resize(qr_array, (qr_size, qr_size), fx=0, fy=0, interpolation=cv2.INTER_AREA)
...
def json_to_agv_info(json_data):
    ...
    return AGV_info(as_dict['length'], as_dict['width'], as_dict['height'], as_dict['serial_no'])
...
class AGV_info:
    def __init__(self, length, width, height, serial_no):
        self.length = length
        self.width = width
        self.height = height
